...
Module ElimE.
  ...
  Section ElimE.
    ...
    Definition efree G := forall A u, (@R Tt Vt) A u el G -> u <> [].
    
    Fixpoint closure (p : symbol -> Prop) {pdec : forall x, dec (p x)} (G : @grammar Tt Vt) :=
      match G with
          [] => []
        | R A u :: Gr =>
          let
    ...
    Section EClosure.
      ...
      Definition eclosed G := forall A u, R A u el G -> forall v, @slist _ (nullable G) _ v u -> R A v el G.
      
      Lemma closure_slist A v (p : symbol -> Prop) {pdec : forall s, dec (p s)} :
        R A v el closure p G -> exists u, R A u el G /\ slist v u.
      Proof.
        intros H.
      ...
      Lemma slist_closure A v (p : symbol -> Prop) {pdec : forall s, dec (p s)} :
        (exists u, R A u el G /\ slist v u) -> R A v el closure p G.
      Proof.
        intros [u [H0 H1]].
    ...
    Section DelE.
      ...
      Proof.
        ...
        - apply slist_split with (xs1 := u) (xs2 := v ++ w) in Ss ; auto.
          ...
          + constructor 2 with (B := B) ; auto using delE_preserveG.
            assert (H2 : @slist _ (nullable G) _ (u1 ++ [Vs B] ++ w1) (u ++ [Vs B] ++ w)).
            { repeat apply slist_append ; auto. }
            apply IHD ; auto.
      ...
      Proof.
        ...
        split ; intros H.
        - destruct H as [H U].
          assert (H0 : @slist _ (nullable G) _ u u) by apply slist_id.
