...
# coq_makefile -f _CoqProject ./int/DFA.v ./int/ChomskyInduction.v ./int/Intersection.v ./int/Base2.v ./int/Union.v ./aut/misc.v ./aut/regexp.v ./aut/glue.v ./aut/re_standard.v ./aut/automata.v ./aut/re_fa.v ./aut/tactics.v ./aut/base.v ./aut/myhill_nerode.v ./aut/transitive_closure.v ./GLL/Definitions.v ./GLL/GLL.v ./cfg/Dec_Empty.v ./cfg/Base.v ./cfg/ElimU.v ./cfg/Lists.v ./cfg/ElimE.v ./cfg/Binarize.v ./cfg/Definitions.v ./cfg/Symbols.v ./cfg/Inlining.v ./cfg/Chomsky.v ./cfg/Dec_Word.v ./cfg/Separate.v ./cfg/Derivation.v ./Boolean_Grammars/ThreeValuedLangDef.v -o Makefile 
#
...
VFILES:=int/DFA.v\
  ...
  cfg/Separate.v\
  cfg/Derivation.v\
  Boolean_Grammars/ThreeValuedLangDef.v
...
html: $(GLOBFILES) $(VFILES)
	- mkdir -p html2
	$(COQDOC) -toc $(COQDOCFLAGS) -html2 $(COQDOCLIBS) -d html2 $(VFILES)
...
install:
	cd "." && for i in $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/fl/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/fl/$$i; \
	done

install-doc:
	install -d "$(DSTROOT)"$(COQDOCINSTALL)/fl/html
	for i in html/*; do \
	 install -m 0644 $$i "$(DSTROOT)"$(COQDOCINSTALL)/fl/$$i;\
	done

uninstall_me.sh: Makefile
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/fl && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "fl" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL)/fl \\\n' >> "$@"
	printf '&& rm -f $(shell find "html" -maxdepth 1 -and -type f -print)\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL) && find fl/html -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	chmod +x $@
